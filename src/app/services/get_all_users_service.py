...
class GetAllUsersService(GetAllUsers):
    ...
    async def get_all(
        ...
        mapped_users = []
        [
            mapped_users.append(UserResponseDTO.model_validate(user))
            for user in found_users.content
        ]
        return Pageable.create(
            mapped_users, limit, offset, found_users.total_elements, len(mapped_users)
        )
